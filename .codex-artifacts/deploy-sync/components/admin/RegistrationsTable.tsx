...
const SMS_BADGE_STYLES: Record<AdminRow["smsStatus"], string> = {
...
function getSmsStatusLabel(value: AdminRow["smsStatus"]) {
  if (value === "SENT") return "Yuborildi";
  if (value === "FAILED") return "Xato";
  return "Kutilmoqda";
}
...
export function RegistrationsTable({
  ...
  const deleteSelectedRows = async () => {
    ...
    try {
      ...
      clearSelection();
      toast.success(`Tanlanganlar o'chirildi (${json?.deletedCount ?? 0})`);
    } catch {
      toast.error("Tarmoq xatosi");
    } finally {
        ...
        {rows.map((r) => (
          <div key={r.id} className="ui-surface space-y-3 p-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant="outline" className={`text-xs ${SMS_BADGE_STYLES[r.smsStatus]}`}>
                SMS: {getSmsStatusLabel(r.smsStatus)}
              </Badge>
            </div>
            <ContactStatusCell
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex h-8 w-8 items-center justify-center rounded-md text-electric-blue hover:bg-electric-blue/10"
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedCount} ta ishtirokchi butunlay o&apos;chiriladi. Bu amalni qaytarib bo&apos;lmaydi.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button variant="destructive" disabled={deletingBulk} onClick={deleteSelectedRows}>
              {deletingBulk ? "O'chirilmoqda..." : "Qat'iy o'chirish"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={Boolean(singleDeleteId)} onOpenChange={(open) => { if (!open) setSingleDeleteId(null); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Bu ishtirokchi butunlay o&apos;chiriladi. Bu amalni qaytarib bo&apos;lmaydi.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button variant="destructive" disabled={deletingSingle} onClick={deleteSingleRow}>
              {deletingSingle ? "O'chirilmoqda..." : "Qat'iy o'chirish"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
